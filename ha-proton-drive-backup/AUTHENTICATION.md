...
> page — the add-on never sees or stores them.

This add-on signs in to Proton Drive through the official `proton-drive` CLI.
Unlike the Google Drive add-on there is no OAuth token, no hosted auth server,
...
To make the CLI usable inside a headless add-on, the container's entrypoint
...
4. Points `HOME` / `XDG_DATA_HOME` at `/data/proton` so the keyring storage and
   the CLI session live on the add-on's persistent volume.

Because everything is anchored under `/data`, you only authenticate **once** —
the session survives add-on restarts and upgrades.

## Signing in

1. Open the add-on's Web UI. While signed out it shows a **Sign in** button.
2. Click **Sign in**. The add-on starts the Proton sign-in for you and shows
...
3. Open that link in any browser — it can be a different device, e.g. your phone —
   ...
   has it enabled. (2FA is handled entirely by Proton's web login; the add-on
...
The add-on captures that URL and surfaces it as the **Sign in** link. The
...
- The keyring key in `/data/proton/keyring.key` only protects the Proton session
  at rest inside the add-on's already-private `/data` volume. Treat a backup of
  `/data` as sensitive — it effectively contains your Proton Drive session.
- The add-on never sees or stores your Proton password; sign-in happens entirely
