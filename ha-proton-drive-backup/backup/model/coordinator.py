...
class Coordinator(Trigger):
    ...
    def markAuthChanged(self):
        # Proton authentication happens out-of-band via the `proton-drive` CLI.
        # The UI calls this right after a successful sign-in, so give the user a
        # short cooldown before automatic backups kick off (mirrors the original
        # add-on's behaviour when credentials were first saved).
        self._global_info.triggerBackupCooldown(timedelta(minutes=self._config.get(Setting.BACKUP_STARTUP_DELAY_MINUTES)))
    ...
    def backupsPausedForProtonAuth(self) -> bool:
        # True when the add-on is creating no backups at all (not even local
        # ones) because Proton upload is enabled but there's no usable Proton
        # session, so the destination counts as "needs configuration".  This is
        # exactly the condition model.sync() uses to skip backup creation, and it
        # lets the updater raise a distinct, actionable "sign in to resume"
        # notification promptly instead of waiting for the generic stale warning.
        if self._global_info._first_sync:
            return False
        return self._model.dest.needsConfiguration()
    ...
    async def _uploadBackup(self, slug):
        ...
        if not backup_dest:
            # Unreachable?
            raise LogicError("This backup isn't in Proton Drive")
