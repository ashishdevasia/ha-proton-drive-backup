...
class SimulatedSource(BackupDestination):
    def __init__(self, name, is_destination=False):
        ...
        self.index = 0
        self.max = 0
        # Imported lazily: backup.model and backup.ha import each other, and a
        # module-level `from ..ha import BackupName` here makes the cycle fail
        # whenever backup.ha is imported before backup.model (e.g. `import
        # backup.ha.harequests`).  Deferring to instantiation breaks the cycle.
