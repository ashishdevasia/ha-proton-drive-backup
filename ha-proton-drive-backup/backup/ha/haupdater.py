...
from .harequests import HaRequests, PAUSED_NOTIFICATION_ID
from ..time import Time
from ..worker import Worker
from ..const import SOURCE_HA, SOURCE_PROTON_DRIVE
from ..logger import getLogger

logger = getLogger(__name__)

NOTIFICATION_TITLE = "Home Assistant Proton Drive Backup is Having Trouble"
...
PAUSED_NOTIFICATION_TITLE = "Proton Drive Backup is paused — sign in needed"
PAUSED_DESC_LINK = "Backups are paused because the add-on is signed out of Proton Drive.  While signed out it creates no new backups at all — not even local Home Assistant ones.  Open the add-on's [Web UI]({0}) and click \"Sign in\" to resume."
PAUSED_DESC_STATIC = "Backups are paused because the add-on is signed out of Proton Drive.  While signed out it creates no new backups at all — not even local Home Assistant ones.  Open the add-on and click \"Sign in\" to resume."
...
class HaUpdater(Worker):
    ...
    async def update(self):
        try:
            ...
            if self._config.get(Setting.NOTIFY_FOR_STALE_BACKUPS):
                await self._updateNotifications()
            self._backoff.reset()
            self._first_error = None
            self._trigger_once = False
    ...
    async def _updateNotifications(self):
        # A signed-out Proton session pauses ALL backup creation (see
        # Coordinator.backupsPausedForProtonAuth), so it gets its own prompt,
        # actionable notification.  It takes precedence over — and suppresses —
        # the generic "having trouble" stale notification, which would otherwise
        # only fire much later and is less specific about the fix.
        paused = self._coordinator.backupsPausedForProtonAuth()
        if paused:
            if self._notified:
            ...
            if not self._paused_notified:
                if self._info.url is None or len(self._info.url) == 0:
                    message = PAUSED_DESC_STATIC
                else:
                    message = PAUSED_DESC_LINK.format(self._info.url)
                await self._requests.sendNotification(
                    PAUSED_NOTIFICATION_TITLE, message, PAUSED_NOTIFICATION_ID)
                self._paused_notified = True
            return

        if self._paused_notified:
            await self._requests.dismissNotification(PAUSED_NOTIFICATION_ID)
            self._paused_notified = False
    ...
    def _buildBackupUpdate(self):
        ...
        proton_backups = list(filter(lambda s: s.getSource(SOURCE_PROTON_DRIVE) is not None, backups))

        last_uploaded = "Never"
        if len(proton_backups) > 0:
            last_uploaded = max(proton_backups, key=lambda s: s.date()).date().isoformat()
        if self._config.get(Setting.CALL_BACKUP_SNAPSHOT):
            return {
                "state": self._state(),
                "attributes": {
                    ...
                    "snapshots_in_proton_drive": len(proton_backups),
                    "snapshots_in_hassio": len(ha_backups),
                    "snapshots_in_home_assistant": len(ha_backups),
                    "size_in_proton_drive": Estimator.asSizeString(sum(map(lambda v: v.sizeInt(), proton_backups))),
            ...
            attr = {
                ...
                "backups_in_proton_drive": len(proton_backups),
                "backups_in_home_assistant": len(ha_backups),
                "size_in_proton_drive": Estimator.asSizeString(sum(map(lambda v: v.sizeInt(), proton_backups))),
                "size_in_home_assistant": Estimator.asSizeString(sum(map(lambda v: v.sizeInt(), ha_backups))),
                "backups": list(map(makeBackupData, backups))
            }
            if SOURCE_PROTON_DRIVE in source_metrics and 'free_space' in source_metrics[SOURCE_PROTON_DRIVE]:
                attr["free_space_in_proton_drive"] = source_metrics[SOURCE_PROTON_DRIVE]['free_space']
            else:
                attr["free_space_in_proton_drive"] = ""
            return {
                "state": self._state(),
                "attributes": attr
            }
