...
from ..config import Config, Setting, Version, ADDON_OPTION_KEYS
...
# Distinct id so the "signed out of Proton, backups paused" repair notification
# is managed independently of the generic "having trouble" one.
PAUSED_NOTIFICATION_ID = "proton_backups_paused"
...
class HaRequests():
    ...
    async def sendNotification(self, title: str, message: str,
                               notification_id: str = NOTIFICATION_ID) -> None:
        data: Dict[str, str] = {
            "title": title,
            "message": message,
    ...
    async def dismissNotification(self, notification_id: str = NOTIFICATION_ID) -> None:
        data: Dict[str, str] = {
            "notification_id": notification_id
        }
    ...
    async def updateConfig(self, config):
        # Supervisor rejects the entire options update if it contains any key
        # the add-on doesn't declare in config.json, so only send declared
        # options.  Internal-only Settings (kept in memory for the model) are
        # dropped here rather than persisted.
        options = {k: v for k, v in config.items() if k in ADDON_OPTION_KEYS}
        dropped = [k for k in config if k not in ADDON_OPTION_KEYS]
        if dropped:
            logger.debug("Not persisting non-schema options to Supervisor: %s", ", ".join(sorted(dropped)))
        return await self._postHassioData(self.getSupervisorURL().with_path("addons/self/options"), {'options': options})
