...
from backup.proton import ProtonSource
...
class BaseModule(Module):
    ...
    @multiprovider
    @singleton
    def getTriggers(self, coord: Coordinator, ha: HaSource, proton: ProtonSource, watcher: Watcher, server: UiServer) -> List[Trigger]:
        return [coord, ha, proton, watcher, server]

    @provider
    @singleton
    def getDestination(self, proton: ProtonSource) -> BackupDestination:
        return proton
    ...
    def getStartables(self, debug_server: DebugServer, ha_updater: HaUpdater, debugger: DebugWorker, ha_source: HaSource,
                      proton: ProtonSource, server: UiServer, restarter: Restarter, syncer: Scyncer, watcher: Watcher, stopper: AddonStopper, precache: Precache) -> List[Startable]:
        # Order here matters, since its the order in which components of the addon are initialized.
        # proton goes before the syncer so the first sync knows whether we're authenticated.
        return [debug_server, ha_updater, debugger, ha_source, proton, server, restarter, syncer, watcher, stopper, precache]
