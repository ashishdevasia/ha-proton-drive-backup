...
### Everything fails with `SyntaxError: JSON Parse error` (even signing in)

A power loss or hard reset can corrupt the Proton CLI's `events.lock` file,
after which every Proton Drive command fails with a log line like:

    WARNING Couldn't verify Proton Drive authentication: ("proton-drive exited 1: ...
    SyntaxError: JSON Parse error ... at ... (src/events/lock.ts:24:29) ...

and signing in again fails the same way. **Since v0.1.3 the add-on repairs
this automatically** — updating the add-on (or restarting it, once updated)
clears the condition.

On older versions, delete the file by hand and restart the add-on. From the
Home Assistant OS host shell:

    rm /mnt/data/supervisor/addons/data/*_hassio_proton_drive_backup/proton/.local/share/proton-drive-cli/events.lock

or, from the SSH add-on with Protection mode disabled:

    docker exec $(docker ps -qf name=hassio_proton_drive_backup) rm /data/proton/.local/share/proton-drive-cli/events.lock

If you changed `proton_data_path`, the lock is at
`<proton_data_path>/.local/share/proton-drive-cli/events.lock` instead.
